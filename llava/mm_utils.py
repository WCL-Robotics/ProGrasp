...
from llava.constants import IMAGE_TOKEN_INDEX, LOC_TOKEN_INDEX, UNSUPERVISED_TOKEN_INDEX, GRASP_Feature_TOKEN_INDEX
...
def tokenizer_special_token_v2(prompt, tokenizer, image_token_index=IMAGE_TOKEN_INDEX, box_token_index=LOC_TOKEN_INDEX, unsupervised_token_index=UNSUPERVISED_TOKEN_INDEX, grasp_feature_token_index=GRASP_Feature_TOKEN_INDEX, return_tensors=None):
    import re
    separators = re.compile(r'(<image>|<boxes>|<unsupervised>|<grasp_feature>)')
    input_ids = []
    offset = 0
    prompt_chunks = separators.split(prompt)  # ['"This is a test ', '<image>', ' with an image ', '<boxes>', ' and some boxes ', '<image>', ' another image']
    first_chunk = tokenizer(prompt_chunks[0]).input_ids
    if first_chunk[0] == tokenizer.bos_token_id:
        ...
        input_ids.append(first_chunk[0])
    for prompt_chunk in prompt_chunks:
        if prompt_chunk not in ['<image>', '<boxes>', '<unsupervised>', '<grasp_feature>']:
        ...
        elif prompt_chunk == '<grasp_feature>':
            input_ids.extend([grasp_feature_token_index])
        else:
            raise NotImplementedError
    if return_tensors is not None:
        if return_tensors == 'pt':
