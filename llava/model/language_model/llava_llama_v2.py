...
from llava.constants import IGNORE_INDEX, GRASP_TOKEN_INDEX, TEPORARY_IGNORE_INDEX, GRASP_LABEL_INDEX

import torch.nn.functional as F

from .modeling_llama_v2 import LlamaForCausalLM_v2
from ..petr_head_v2 import PETRHead_v2
# from ..petr_head import PETRHead_v2
...
class LlavaLlamaForCausalLM_v2(LlamaForCausalLM_v2, LlavaMetaForCausalLM):
    ...
    def prepare_inputs_for_generation(self, input_ids, gs, past_key_values=None, attention_mask=None,
                                    inputs_embeds=None, images=None, pure_imgs=None,
                                    depths=None, poses=None, intrinsics=None, lengths=None,
                                    pcs=None, clicks=None, image_sizes=None, **kwargs):
        if past_key_values is None:
            (input_ids, position_ids, attention_mask, past_key_values, inputs_embeds, labels) = \
                self.prepare_inputs_labels_for_multimodal(
                    input_ids, gs, None, attention_mask, None, None,
                    images, pure_imgs, depths, poses, intrinsics, lengths, pcs, clicks, image_sizes
                )
            return {
        ...
        return {
            "input_ids": input_ids,
            "attention_mask": None, # 让模型自动生成匹配 KV Cache 长度的 mask
            "past_key_values": past_key_values,
            "use_cache": kwargs.get("use_cache", True),
        }
    ...
    def forward(
        ...
        gs:Optional[List[torch.FloatTensor]] = None,
        ...
        grasp_mask = (labels == GRASP_LABEL_INDEX)
        # if labels != None:
        #     for i in range(grasp_mask.shape[1]):
        #         if grasp_mask[0][i]==True:
        #             print("find grasp label position:", i)
        ...
        if grasp_out:
            ...
            masked_scores = []
            for i in range(B):
                masked_scores.append(grasp_outs['all_cls_scores'][0][i][grasp_mask[i]])
            grasp_outs['all_cls_scores'] = torch.stack(masked_scores).unsqueeze(0)
            return grasp_outs
        ...
        for i in range(B):
            valid_inds = (labels[i]!=GRASP_TOKEN_INDEX) & (labels[i]!=IGNORE_INDEX) & (labels[i]!=GRASP_LABEL_INDEX)
