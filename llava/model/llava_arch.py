...
from .multimodal_encoder.builder import build_vision_tower, build_video_tower, build_voxel_tower, build_grasp_tower
from .multimodal_projector.builder import build_vision_projector

from llava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, GRASP_LABEL_INDEX, GRASP_Feature_TOKEN_INDEX, GRASP_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, LOC_TOKEN_INDEX
...
class LlavaMetaModel:
    ...
    def get_grasp_tower(self):
        grasp_tower = getattr(self, 'grasp_tower', None)
        if type(grasp_tower) is list:
            grasp_tower = grasp_tower[0]
        return grasp_tower
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        grasp_tower = getattr(model_args, 'grasp_tower', None)
        ...
        self.config.mm_grasp_tower = grasp_tower
        if grasp_tower is not None:
            if self.get_grasp_tower() is None:
                grasp_tower = build_grasp_tower(model_args)

                if fsdp is not None and len(fsdp) > 0:
                    self.grasp_tower = [grasp_tower]
                else:
                    self.grasp_tower = grasp_tower
            else:
                if fsdp is not None and len(fsdp) > 0:
                    grasp_tower = self.grasp_tower[0]
                else:
                    grasp_tower = self.grasp_tower
...
class LlavaMetaForCausalLM(ABC):
    ...
    def get_grasp_tower(self):
        return self.get_model().get_grasp_tower()
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            num_grasps = (cur_input_ids == GRASP_Feature_TOKEN_INDEX).sum()
            num_specials = num_images + num_prompts + num_grasps
            # ------------------------------------------------------
            if num_images == 0:
            ...
            grasp_token_indices = torch.where(cur_input_ids == GRASP_Feature_TOKEN_INDEX)[0].tolist()
            special_token_indices = sorted(image_token_indices + prompt_token_indices + grasp_token_indices)
            ...
            for i in range(num_specials + 1):  # num_images = 1? [0, 1]
                ...
                if i < num_specials:
                    ...
                    elif special_token == GRASP_Feature_TOKEN_INDEX:
                        cur_grasp_features = grasp_features[cur_grasp_idx]  # (N, C)
                        cur_grasp_idx += 1
                        cur_new_input_embeds.append(cur_grasp_features)
                        cur_new_labels.append(torch.full((cur_grasp_features.shape[0],), GRASP_LABEL_INDEX, device=cur_labels.device, dtype=cur_labels.dtype))
                    else:
                        raise NotImplementedError
