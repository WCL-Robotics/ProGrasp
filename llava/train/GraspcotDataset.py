...
class GraspcotDataset_Train(Dataset):
    ...
    def get_data_info(self, index):
        ...
        for k, obj_id in enumerate(used_dia_objs):
            if obj_id >= len(grasps):
                print(f"Warning: obj_id {obj_id} is out of bounds for grasps list (len={len(grasps)}) in scene {scene}. Skipping this object.")
                continue
            pos_grasps.append(grasps[obj_id])
            pos_gs_labels.append(gs_labels[obj_id]-1)
            pos_grasp_ids.append(scene+"_"+str(obj_id))

        # Check if we have any valid grasps
        if not pos_grasps:
            print(f"Error: No valid grasps found for scene {scene} after filtering.")
            print(f"used_dia_objs: {used_dia_objs}")
            # Return a random valid sample instead
            return self.get_data_info((index + 1) % len(self.aligned_infos['infos']))
...
class GraspcotDataset_Test(Dataset):
    ...
    def get_data_info(self, index):
        ...
        # mask_response_str_ = "_ " * 30
        # mask_response_str_ = mask_response_str_[:-1]
        # print("mask_response_str_:", len(mask_response_str_))
